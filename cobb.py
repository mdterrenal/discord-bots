...
@bot.command(name='r', help='Rolls dice with a specified number of sides.')
...
async def describe_spell(ctx, *spell_name):
    ...
    try:
        ...
        await ctx.send('There was some error in retrieving the information. '
    ...
    if extracted_data['count'] > 1:
        ...
        if no_match_exists:
            await ctx.send('There are multiple results for that. '
                           'Did you possibly mean any of these spells:\n'
                           + '\n'.join(spell_names))
    elif extracted_data['count'] == 1:
        await ctx.send(provide_spell_description(extracted_data['results'][0]))
    elif extracted_data['count'] == 0:
        await ctx.send('It seems that there is no matching spell for this. '
                       'You might have misspelled the name. Please try again.')


@bot.command(name='spell', help='Provides all spells '
             'of a specified level and class.')
async def output_list_of_spells(ctx, level: int, dnd_class):
    payload = {'format': 'json', 'level_int': level,
               'dnd_class__icontains': dnd_class.capitalize()}
    url = 'https://api.open5e.com/spells'
    try:
        response = requests.get(url, params=payload)
        response.raise_for_status()
    ...
    if extracted_data['count'] > 1:
        ...
        for result in results:
            spell_names.append(result['name'])
        await ctx.send('Here is a list of all ' + str(level)
                       + level_endings.get(str(level), 'th')
                       + ' level ' + dnd_class.capitalize()
                       + ' spells:\n' + '\n'.join(spell_names))
    elif extracted_data['count'] == 0:
        await ctx.send('It seems that there are no matching spells for this. '
                       'Please try again.')
...
async def describe_monster_stats(ctx, *monster_name):
    ...
    if extracted_data['count'] > 1:
        ...
        if no_match_exists:
            await ctx.send('There are multiple results for that. '
                           'Did you possibly mean any of these monsters:\n'
                           + '\n'.join(monster_names))
    elif extracted_data['count'] == 1:
        await ctx.send(provide_monster_stats(extracted_data['results'][0]))
    elif extracted_data['count'] == 0:
        await ctx.send('It seems that there is no matching monster for this. '
                       'You might have misspelled the name. Please try again.')


@bot.command(name='commands', help='Provide a list of'
             'all the commands that the bot accepts.')
async def provide_command_list(ctx):
    await ctx.send('Here is a list of all of the commands that you can use:'
                   '\n!okay\n!smh\n!dank\n!r\n!cast\n!spell\n!commands\n'
                   'Use !help command name to learn about a specific command.')


def provide_spell_description(response):
    final_desc = [response['name'], response['desc'], response['range'],
                  response['components'], response['duration'],
                  'Concentration: ' + response['concentration'].capitalize(),
                  response['casting_time'], response['level']]


def provide_monster_stats(response):
    values_format = 'STR: {0}   DEX: {1}   CON: {2}   '\
                    'INT: {3}   WIS: {4}   CHA: {5}'
    ability_values = values_format.format(str(response['strength']),
                                          str(response['dexterity']),
                                          str(response['constitution']),
                                          str(response['intelligence']),
                                          str(response['wisdom']),
                                          str(response['charisma']))
    final_desc = [response['name'], response['size'] + ' '
    ...
    if response['damage_vulnerabilities'] != "":
        final_desc.append('Damage Vulnerabilities: '
                          + response['damage_vulnerabilities'])
    if response['damage_resistances'] != "":
        final_desc.append('Damage Resistances: '
                          + response['damage_resistances'])
    if response['damage_immunities'] != "":
        final_desc.append('Damage Immunities: '
                          + response['damage_immunities'])
    if response['condition_immunities'] != "":
        final_desc.append('Condition Immunities: '
                          + response['condition_immunities'])
